...
Route::get('send_notif', 'PublicController@send_notif');
...
Route::post('verifierdocument', function () {
	$user = DB::table('users')->where('id', $_POST['user_id'])->first();
		if($user->validation_document == '1'){
			DB::table('users') ->where('id', $user->id) ->update(['validation_document' => 0]);
		}else{
			DB::table('users') ->where('id', $user->id) ->update(['validation_document' => 1]);
		}
});

Route::post('upload-cine', function () {

$msg['status'] = 'false';

$target_dir = "uploads/cnie/";
$target_file = $target_dir . md5(uniqid()).basename($_FILES["file"]["name"]);
$uploadOk = 1;
...
if(isset($_POST["submit"])) {
  ...
  if($check !== false) {
    $msg['msg'] = "Le fichier est une image - " . $check["mime"] . ".";
    $uploadOk = 1;
  } else {
...
if (file_exists($target_file)) {
  $msg['msg'] = "Désolé, le fichier existe déjà..";
  $uploadOk = 0;
}

// Check file size
if ($_FILES["file"]["size"] > 500000) {
  $msg['msg'] = "Désolé, votre fichier est trop volumineux.";
  $uploadOk = 0;
}
...
&& $imageFileType != "gif" ) {
  $msg['msg'] = "Désolé, seuls les fichiers JPG, JPEG, PNG et GIF sont autorisés.";
  $uploadOk = 0;
}

// Check if $uploadOk is set to 0 by an error
if ($uploadOk == 0) {
  $msg['msg'] = "Désolé, votre fichier n'a pas été téléchargé.";
// if everything is ok, try to upload file
} else {
  if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
	session(['cnie' => $target_file]);
    $msg['msg'] = $target_file;
	$msg['status'] = 'true';
  } else {
    $msg['msg'] = "Désolé, une erreur s'est produite lors du téléchargement de votre fichier.";
  }
}
...
// Admin Routes
Route::group(
    ['middleware' => ['role:moderateur|admin']],
    function () {
...
Route::group(
    ...
    function () {
        Route::post('change_role', 'UserController@change_role')->name('change_role');
...
Route::group(
    ...
    function () {
        ...
        Route::post('user/submit-review', 'UserController@submitReview')->name('review');
...
Route::get('/redireCategorie','CategoryController@redireCategorie')->name('redireCategorie');

Route::get('/privacy-policy','HomeController@privacyPolicy')->name('privacyPolicy');
Route::get('/mentions-legals','HomeController@mentionsLegals')->name('mentionsLegals');
Route::get('/traitement-des-donnees-personnelles','HomeController@traitement')->name('traitement-des-donnees-personnelles');
Route::get('/cgv','HomeController@cgv')->name('cgv');
Route::get('/payment-cmi/{amount}','UserController@cmigateaway')->name('payment-cmi');
Route::post('/process','UserController@process')->name('process');
Route::get('/okfail','UserController@okfail')->name('okfail');