...
//		socket_write($sock, $commande, strlen($)) or die("Could not send data to server\n");



        if(preg_match("/^220/i", $out = socket_read($this->connect, 2048, PHP_BINARY_READ))){
...
//          $out = fgets ($this->connect);
			...
			$commande = "HELO ".$this->get_domain($this->verifier_email)."\r\n";
			socket_write($this->connect, $commande, strlen($commande)) or die("Could not send data to server\n");
			$out = socket_read($this->connect, 2048, PHP_BINARY_READ);
 ...
 //         $from = fgets ($this->connect);
			...
			$commande = "MAIL FROM: <".$this->verifier_email.">\r\n";
			socket_write($this->connect, $commande, strlen($commande)) or die("Could not send data to server\n");
		    $from = socket_read($this->connect, 2048, PHP_BINARY_READ);		  
...
//          $to = fgets ($this->connect);
			...
			$commande = "RCPT TO: <".$this->email.">\r\n";
			socket_write($this->connect, $commande, strlen($commande)) or die("Could not send data to server\n");
			$to = socket_read($this->connect, 2048, PHP_BINARY_READ);				  
		  

          $this->debug_raw['rcpt_to'] = $to;
          $this->debug['takchbila'] = 'Response: '.$to;
...
//          $quit = fputs ($this->connect , "QUIT");
			...
			$commande = "QUIT";
			socket_write($this->connect, $commande, strlen($commande)) or die("Could not send data to server\n");
//			$quit = socket_read($this->connect, 2048, PHP_BINARY_READ);	
...
//          $this->debug_raw['quit'] = "quit";
    ...
    private function connect_mx($ips) {
      //connect
 //     $this->connect = @fsockopen($this->mx, $this->port);

		$this->connect = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
		socket_bind($this->connect, $ips);
		socket_connect($this->connect, $this->mx, $this->port);
