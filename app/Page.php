 ...
 * @license http://elmoukef.ma Amentotech
 * @link    http://elmoukef.ma
 */
...
class Page extends Model
{
    ...
    public function savePage($request)
    {
        if (!empty($request)) {
            ...
            if (!empty($request['meta'])) {
                foreach ($request['meta'] as $key => $value) {
                    if (!empty($value)) {
                        if ($key == 'headings' || $key == 'freelancers' || $key == 'cat' || $key == 'services' || $key == 'articles') {
                        ...
                        } elseif ($key == 'jobs' && !empty($value)) {
                            foreach ($value as $meta_key => $meta_value) {
                                $job_section = array();
                                $meta = new Meta();
                                foreach ($meta_value as $banner_key => $banner_value) {
                                    if ($banner_key == 'backgroundImg' && !empty($banner_value)) {
                                        if (file_exists($old_path . '/' . $banner_value)) {
                                            if (!file_exists($new_path)) {
                                                File::makeDirectory($new_path, 0755, true, true);
                                            }
                                            $filename = time() . '-' . $banner_value;
                                            rename($old_path . '/' . $banner_value, $new_path . '/' . $filename);
                                        } else {
                                            $filename = $banner_value;
                                        }
                                        $job_section[$banner_key] = $filename;
                                    } else {
                                        $job_section[$banner_key] = $banner_value;
                                    }
                                }
                                $meta->meta_key = $key . (string) $value[$meta_key]['parentIndex'];
                                $meta->meta_value = serialize($job_section);
                                $page->meta()->save($meta);
                            }
                        } elseif ($key == 'packages' && !empty($value)) {
                            foreach ($value as $meta_key => $meta_value) {
                                $pkg_section = array();
                                $meta = new Meta();
                                foreach ($meta_value as $banner_key => $banner_value) {
                                    if ($banner_key == 'backgroundImg' && !empty($banner_value)) {
                                        ...
                                        $pkg_section[$banner_key] = $filename;
                                    } else {
                                        $pkg_section[$banner_key] = $banner_value;
                                    }
                                }
                                $meta->meta_key = $key . (string) $value[$meta_key]['parentIndex'];
                                $meta->meta_value = serialize($pkg_section);
                                $page->meta()->save($meta);
                            }
                        } elseif ($key == 'categoriesSecondVersion' && !empty($value)) {
                            foreach ($value as $meta_key => $meta_value) {
                                $cat_section = array();
                                $meta = new Meta();
                                foreach ($meta_value as $banner_key => $banner_value) {
                                    if ($banner_key == 'backgroundImg' && !empty($banner_value)) {
                                        ...
                                        $cat_section[$banner_key] = $filename;
                                    } else {
                                        $cat_section[$banner_key] = $banner_value;
                                    }
                                }
                                $meta->meta_key = $key . (string) $value[$meta_key]['parentIndex'];
                        ...
                        } elseif ($key == 'categoriesThirdVersion' && !empty($value)) {
                            foreach ($value as $meta_key => $meta_value) {
                                $cat_section = array();
                                $meta = new Meta();
                                foreach ($meta_value as $banner_key => $banner_value) {
                            ...
                            foreach ($value as $meta_key => $meta_value) {
                                ...
                                foreach ($meta_value as $banner_key => $banner_value) {
                                    if ($banner_key == 'backgroundImg' && !empty($banner_value)) {
                                        ...
                                        $banner_section[$banner_key] = $filename;
                                    } elseif ($banner_key == 'frontImg' && !empty($banner_value)) {
                                        if (file_exists($old_path . '/' . $banner_value)) {
                                            if (!file_exists($new_path)) {
                                                File::makeDirectory($new_path, 0755, true, true);
                                            }
                                        ...
                                        $banner_section[$banner_key] = $banner_value;
                                    }
                                }
                                $meta->meta_key = $key . (string) $value[$meta_key]['parentIndex'];
                                $meta->meta_value = serialize($banner_section);
                                $page->meta()->save($meta);
                            }
                            ...
                            foreach ($value as $meta_key => $meta_value) {
                                ...
                                foreach ($meta_value as $app_key => $app) {
                                    if (($app_key == 'background_image' || $app_key == 'app_image' || $app_key == 'ios_image' || $app_key == 'android_image') && !empty($app)) {
                            ...
                            foreach ($value as $meta_key => $meta_value) {
                                ...
                                foreach ($meta_value as $work_tab_key => $work_tab) {
                                    if (($work_tab_key == 'background_image' || $work_tab_key == 'first_tab_icon' || $work_tab_key == 'second_tab_icon' || $work_tab_key == 'third_tab_icon') && !empty($work_tab)) {
            ...
            if (!empty($request['headerStyling'])) {
                $headerStyling = array();
                foreach ($request['headerStyling'] as $header_key => $value) {
                    if ($header_key == 'logo' && !empty($value)) {
                        if (file_exists($old_path . '/' . $value)) {
                            if (!file_exists($new_path)) {
                                File::makeDirectory($new_path, 0755, true, true);
                            }
                            $filename = time() . '-' . $value;
                            rename($old_path . '/' . $value, $new_path . '/' . $filename);
                            $headerStyling[$header_key] = $filename;
                        }
                    } else {
                        $headerStyling[$header_key] = $value;
                    }
                }
                $meta = new Meta();
                $meta->meta_key = 'header_styling';
                $meta->meta_value = serialize($headerStyling);
                $page->meta()->save($meta);
            }
            ...
            if (!empty($request['meta_title'])) {
                DB::table('site_managements')->insert(
                    [
                        'meta_key' => 'meta-title-' . $page_id, 'meta_value' => $request['meta_title'],
                        "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                    ]
                );
            }
    ...
    public function updatePage($id, $request)
    {
        ...
        if (!empty($id) && !empty($request)) {
            ...
            if (!empty($request['meta_title'])) {
                DB::table('site_managements')->where('meta_key', '=', 'meta-title-' . $id)->delete();
                DB::table('site_managements')->insert(
                    [
                        'meta_key' => 'meta-title-' . $id, 'meta_value' => $request['seo_desc'],
                        "created_at" => Carbon::now(), "updated_at" => Carbon::now()
                    ]
                );
            }
