...
class UserController extends Controller
{
    ...
    public function submitTransection(Request $request)
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($type) && !empty($product_id)) {
                ...
                $json['type'] = 'success';
                //  $json['return_url'] = url('/payment-cmi/'.$product_price );
                $json['return_url'] = route('payment-cmi',['amount'=>$product_price]);
    ...
    public function change_role(Request $request){
			$newrole = json_decode($_POST['new_role'],true);

            $user = User::find($newrole['id_user']);
//            $user->name     = $request->name;
//            $user->save();
 $user->roles()->sync([$newrole['role']]);

			
			
		//echo $newrole['id_user'];
		//echo $newrole['role'];
    ...
    public function userListing(Request $request)
    {
		...
		//$role->roles()->sync([$request->input('2')]);
...
public function process(Request $req)
{
    ...
    $client = new CmiClient([
        'storekey' => 'Hardlimp5', // STOREKEY
        'clientid' => '600003046', // CLIENTID
        'oid' => uniqid(), // COMMAND ID IT MUST BE UNIQUE
        ...
        'email' => 'mohamedkoubou@hotmail.com', // YOUR EMAIL APPEAR IN CMI PLATEFORM
        'BillToName' => 'mohamed koubou', // YOUR NAME APPEAR IN CMI PLATEFORM
        'BillToCompany' => 'MSNA SERVICES SARL', // YOUR COMPANY NAME APPEAR IN CMI PLATEFORM
        ...
        'tel' => '+212667412124', // YOUR PHONE APPEAR IN CMI PLATEFORM NOT REQUIRED
        'amount' => $req->amount, // RETRIEVE AMOUNT WITH METHOD POST
...
{
    return view('back-end.employer.dashboard');
}


public function cmigateaway(Request $req)
{
   $amount = $req->amount;
    
             return view('back-end.employer.payouts.cmi',compact('amount'));
}
