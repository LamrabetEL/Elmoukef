...
class RestAPIController extends Controller
{
    ...
    public function getEmployer()
    {
        ...
        if ($type === 'latest') {
            ...
            if (!empty($save_employer)) {
                $users = User::whereIn('id', $save_employer)->orderBy('updated_at', 'desc')
                    ->paginate($post_per_page);
            } else {
                unset($json[0]);
                $json['type']        = 'error';
    ...
    public function taxonomies()
    {
        ...
        if (!empty($type)) {
            ...
            if ($type == 'delivery_time') {
                ...
                return Response::json($delivery_time, 200);
            }
            // delivery time
            if ($type == 'response_time') {
                $response_time = ResponseTime::all();
                return Response::json($response_time, 200);
            }
    ...
    public function getSettings()
    {
        ...
        if (!empty($type)) {
            ...
            } elseif ($type == 'chat-settings') {
                $chat_settings = SiteManagement::getMetaValue('chat_settings');
                $json['chat_settings'] = $chat_settings;
            }
            return Response::json($json, 200);
        } else {
            $json['type']    = 'error';
    ...
    public function getServices()
    {
        ...
        if ($type === 'latest') {
            ...
            if (!empty($saved_services)) {
                $services =  Service::whereIn('id', $saved_services)->paginate($post_per_page);
            } else {
                unset($json[0]);
                $json['type']     = 'error';
                $json['message']  = trans('lang.no_services_list');
                return Response::json($json, 203);
            }
        } elseif ($type === 'search') {
            $json[0]['count'] = Service::count();
    ...
    public function postService(Request $request)
    {
        ...
        if ($package_status === 'true') {
            ...
            if (!empty($option['no_of_services']) && $posted_services >= intval($option['no_of_services'])) {
                ...
                $service_post = $service->storeAPIService($request, $current_user, $image_size);
    ...
    public function getChatUsers()
    {
        ...
        if (!empty($users)) {
            foreach ($users as $key => $userVal) {
                ...
                $json[$key]['image'] = url(Helper::getProfileImage($chat_user_id));
    ...
    public function storeMessages(Request $request)
    {
        ...
        if (!empty($request)) {
            ...
            $messsage = $messsage->sendMessage($request);
            $json['type']        = 'success';
