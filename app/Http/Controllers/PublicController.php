...
class PublicController extends Controller
{
    ...
    {
		if(!empty($_POST['rc'])){
			Session::put('rc', $_POST['rc']);
		}
		
		if(!empty($_POST['ice'])){
			Session::put('ice', $_POST['ice']);
		}
		
		if(!empty($_POST['num'])){
			Session::put('num', $_POST['num']);
		}
        $this->validate(
            $request,
            [
	...
	public function send_notif(){
		...
		$job = Job::where('send', 0)->first();
		if(!empty($job->id)){
			DB::table('jobs') ->where('id', $job->id) ->update(['send' => 1]);
			//$job->location_id;
			//print_r($job->categories);
			$arra_user = array();
			$cattt = array();
			$categorie = DB::table('catables')->where('catable_id', $job->id)->where('catable_type', "App\Job")->get();
			foreach ($categorie as $cat)
			{
				// $cat->category_id;
				$users = DB::table('catables')->where('category_id', $cat->category_id)->where('catable_type', "App\Profile")->get();
				foreach($users as $id_user){
					$arra_user[] = $id_user->catable_id;
				}
			}
			...
			$email_params = array();
			$email_params['job_title'] = "titree";
			$email_params['posted_job_link'] = "URL";
			$email_params['name'] = "Name";
			$email_params['link'] = "link";
									
									
				$result = array_unique($arra_user);
				foreach($result as $arr){
					if(!empty($arr)){
					$user = DB::table('users')->where('id', $arr)->where('location_id', $job->location_id)->first();
					Mail::to($user->email)->send( new EmployerEmailMailable( 'employer_email_new_job_posted',  $template_data_employer, $email_params ));
					}
				}
