...
class User extends Authenticatable
{
    ...
    public function storeUser($request, $verification_code, $registration_type = '', $verification_type = '')
    {
        if (!empty($request)) {
			...
			if(!empty(Session::get('cnie'))){
				$this->document = filter_var(Session::get('cnie'), FILTER_SANITIZE_STRING);
				$this->type_document = 'particulier';
				Session::forget('cnie');
			}elseif(!empty(Session::get('rc'))){
				$this->document = filter_var(Session::get('rc'), FILTER_SANITIZE_STRING);
				$this->ice = filter_var(Session::get('ice'), FILTER_SANITIZE_STRING);
				$this->type_document = 'prefessionnel';
				Session::forget('rc');
				Session::forget('ice');
			}
			
			if(!empty(Session::get('num'))){
				$this->numero = filter_var(Session::get('num'), FILTER_SANITIZE_STRING);
				Session::forget('num');
			}
			
            $this->validation_document = 0;
